...
- iOS 26 灵动岛与锁屏实时播放信息：显示歌曲封面、歌曲名和歌手，播放/暂停与进度同步；后台音频模式和系统媒体控制支持切到其他 App 后继续播放
...
- 本地 AI 歌单：内置 `present_choice` AI 工具，由 DeepSeek V4 Pro 主动决定问题和可点击选项；创建时读取真实“我喜欢的音乐”作为审美上下文，最多生成 50 首候选，再严格筛选用户指定数量；深度思考与方案采用限频流式展示，长文本不会越生成越卡；用户主动创建后使用 iOS 26 持续后台任务，切到其他 App 仍可继续，歌单仅保存于 App 本机
